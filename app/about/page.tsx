  ...
  return (
    <main className="bg-zinc-50 min-h-screen px-6 flex flex-col justify-center lg:px-55 py-12 gap-10">
        <section className='flex flex-col lg:flex-row gap-y-12 my-19 lg:gap-x-10 lg:items-center'>
          ...
          <h2 className='text-zinc-800 lg:text-[60px] font-medium text-center'>Sobre mi</h2>
...
Hoy me defino como Frontend Developer con foco en UI — diseño interfaces en Figma y las llevo a código con atención obsesiva al detalle y fidelidad al pixel perfect. Me importa que lo que se ve en pantalla sea exactamente lo que fue pensado. Ni más, ni menos.
        ...
        <section className='lg:flex lg:flex-col lg:justify-between'>
            <article>
  <h3 className='text-zinc-800 text-[35px] lg:text-[45px] mb-6'>Educación</h3>
  {dataEducacion.map((item) => (
    <div key={item.year + item.curso} className='flex items-center gap-4 leading-14'>
      <span className="text-zinc-700 h-8 w-20 bg-white/15 background-blur-sm rounded-lg
       font-bold flex justify-center items-center lg:h-8.75 lg:w-20 text-[22px] lg:text-[25px]">
        {item.year}
      </span>
      <p className='text-[20px] text-zinc-500'>{item.curso} - <span className='text-zinc-700'>{item.institucion}</span></p>
        ...
        <article>
  <h3 className='text-zinc-800 text-[35px] lg:text-[45px] my-6'>Experiencia</h3>
    ...
    <div className='flex items-center gap-4 mb-10'>
      ...
      <p className='text-[20px] text-zinc-500'>Aikodev - <span className='text-zinc-700'>Desarrolador Frontend - Diseñador UX/UI</span> </p>
    </div>
</article>
<div
